...
public class CAA_EvadeStateRBS : CAA_BaseStateRBS
{
    ...
    public override Type StateUpdate()
    {
        ...
        if (smartTank.VisibleEnemyTanks.Count > 0)                                                                   
        {
            ...
            var closestEnemy = smartTank.VisibleEnemyTanks.OrderBy(e => Vector3.Distance(smartTank.transform.position, e.Key.transform.position)).First().Key;

            // move away from enemy
            Vector3 directionAway = (smartTank.transform.position - closestEnemy.transform.position).normalized;

            Vector3 evadeTarget = smartTank.transform.position + directionAway * evadeDistance;
